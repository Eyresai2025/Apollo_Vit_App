...
BASE_SRC_DIR = Path(__file__).resolve().parents[1]   # points to src
TRAINING_DIR = BASE_SRC_DIR / "training"
VIT_TRAINING_ROOT = str(TRAINING_DIR / "VIT_Training")
YOLO_R_PATH = str(TRAINING_DIR / "best (1) 1.pt")
...
class NewSKUPage(QWidget):
    def __init__(
        ...
        # Load raw images for preview on startup
    ...
    def load_raw_images_for_preview(self):
        """Load existing images from raw_dir for preview"""
        if self.capture_in_progress or self.training_in_progress:
            return
        
        # Clear existing preview paths
        self.latest_preview_paths = {}
        
        # Define camera serials in order (matching the 4 images)
        camera_serials = ["254701283", "254701301", "254701300", "254701292"]
        
        # Try to load images from raw_dir
        if os.path.exists(self.raw_dir):
            # Look for image files in the raw directory
            image_files = []
            for file in os.listdir(self.raw_dir):
                if file.lower().endswith(IMAGE_EXTS):
                    image_files.append(file)
            
            # Sort images by name to maintain order (1.jpg, 2.jpg, 3.jpg, 4.jpg)
            image_files.sort()
            
            # Map the images to camera positions in order
            for idx, serial in enumerate(camera_serials):
                if idx < len(image_files):
                    ...
                    # If not enough images, try to find any image with matching pattern
        ...
        # If still no images found, try to load by camera serial pattern
    ...
    def _set_camera_status(self, serial: str, text: str, color: str = "#333"):
    ...
    def _reset_training_cards(self):
        for serial in ["254701283", "254701301", "254701300", "254701292"]:
            if CAMERA_PIPELINE_MAP.get(serial):
                self._set_camera_status(serial, "Status: Waiting", "#6f6a7a")
            else:
                self._set_camera_status(serial, "Status: Not configured", "#b3aac5")
    ...
    def _set_serial_progress(self, serial: str, frac: float):
        ...
        total_frac = sum(self.serial_stage_progress.values()) / len(self.enabled_training_serials)
    ...
    def _update_training_card_from_log(self, msg: str):
        if self.training_current_action_lbl is not None:
            self.training_current_action_lbl.setText(f"Current action: {msg}")

        m = re.search(r"\[TRAIN\] serial=(\d+)", msg)
        if m:
            serial = m.group(1)
            self.active_training_serial = serial
            self._set_camera_status(serial, "Status: Preparing dataset...", "#b16e2c")
            self._set_serial_progress(serial, 0.05)
            return

        if "[PIPELINE] Dataset preparation complete." in msg and self.active_training_serial:
            self._set_camera_status(self.active_training_serial, "Status: Dataset ready. Training started...", "#1f6390")
            self._set_serial_progress(self.active_training_serial, 0.35)
            return

        m = re.search(r"Epoch \[(\d+)/(\d+)\]", msg)
        if m and self.active_training_serial:
            ep = int(m.group(1))
            total = int(m.group(2))
            frac = 0.35 + 0.55 * (ep / max(total, 1))
            self._set_camera_status(
                self.active_training_serial,
                f"Status: Training epoch {ep}/{total}",
                "#1f6390"
            )
            self._set_serial_progress(self.active_training_serial, frac)
            return

        m = re.search(r"\[DONE\] serial=(\d+)", msg)
        if m:
            serial = m.group(1)
            self._set_camera_status(serial, "Status: Completed", "#2a6e2f")
            self._set_serial_progress(serial, 1.0)
            return

        m = re.search(r"\[FAIL\] serial=(\d+)", msg)
        if m:
            serial = m.group(1)
            self._set_camera_status(serial, "Status: Failed", "#bd3b2b")
            self._set_serial_progress(serial, 1.0)
            return

        m = re.search(r"\[SKIP\] serial=(\d+)", msg)
        if m:
            serial = m.group(1)
            self._set_camera_status(serial, "Status: Skipped", "#9a92ae")
            return
    ...
    def refresh_preview_with_raw_load(self):
        """Manual refresh that loads raw images if available"""
        if self.capture_in_progress or self.training_in_progress:
            return
        
        # First try to load from raw_dir
        self.load_raw_images_for_preview()
    ...
    def start_training(self):
        ...
        self.training_summary_lbl.setText("Training is running for configured camera pipelines.")
    ...
    def _on_training_status(self, msg: str):
        ...
        self.training_summary_lbl.setText("Training is in progress for configured cameras.")
        self.status_lbl.setText("Training running...")
