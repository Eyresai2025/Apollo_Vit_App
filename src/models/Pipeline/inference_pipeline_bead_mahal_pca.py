...
from src.COMMON.common import bead_dimensions
from src.models.defect_dimension import cor_bead, area_defect_bead
...
def enrich_bead_yolo_df_with_dimensions(seg_df, crop_path, tyre_name, debug_save_dir=None):
    dim_summary = {
        "tyre_name": tyre_name,
    ...
    if crop_path is None or not os.path.isfile(crop_path):
        print("[DIM][WARN] crop_path missing, skipping bead dimension calculation")
        return seg_df, dim_summary
    ...
    if not tyre_name_normalized:
        print("[DIM][WARN] tyre_name missing, skipping bead dimension calculation")
        return seg_df, dim_summary

    try:
        bead_width_mm, bead_height_mm, bead_area_mm2 = bead_dimensions(tyre_name_normalized)
        dimension_basis = "bead_dimensions"
    except Exception as e:
    ...
    out_df["bead_width_mm"] = float(bead_width_mm)
    out_df["bead_height_mm"] = float(bead_height_mm)
    out_df["bead_area_mm2"] = float(bead_area_mm2)
    ...
    for _, row in out_df.iterrows():
        rdlen_mm, rdwid_mm = cor_bead(
            iwid=base_w,
            ilen=base_h,
            dwid=float(row["bbox_width_px"]),
            dlen=float(row["bbox_height_px"]),
            beadHeight=bead_height_mm,
            beadWidth=bead_width_mm,
        )

        area_mm2 = area_defect_bead(
            t_pix=total_pix,
            d_pix=float(row["bbox_area_px"]),
            areaOfBead=bead_area_mm2,
        )
...
def run_yolo_on_vit_defect_patches(
    ...
    if not seg_df.empty:
        ...
        seg_df, dim_summary = enrich_bead_yolo_df_with_dimensions(   # * = sidewall/innerwall/etc.
            seg_df=seg_df,
            crop_path=crop_path,
            tyre_name=tyre_name,
            debug_save_dir=None,
...
def load_calibration_artifacts_from_dir(output_dir, ref_image_path_override=None):
    ...
    Priority:
    1. If ref_image_path_override is passed from Maincycle,
       use its folder as artifacts dir:
       media/calibration/<SKU_NAME>/artifacts/

    2. Otherwise fallback to old:
       output_dir/calibration/artifacts/
    """
...
def get_patch_embeddings_batched(model, patch_records_by_side, device, tfm=None):
    ...
    Args:
        model: ViT model
        patch_records_by_side: dict {side_name: [patch_records]}
        device: device
        tfm: transform
    
    Returns:
        embeddings_by_side: dict {side_name: tensor[N_side, D]}
        valid_records_by_side: dict {side_name: [valid_records]}
    """
    if tfm is None:
        tfm = _build_transform()
    
    # Collect all valid patches with their side info
    all_imgs = []
    all_metadata = []  # (side_name, original_index, record)
