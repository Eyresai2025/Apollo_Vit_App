...
CALIBRATION_DIR_NAME = "calibration_innerwall"
# with 5 images total:
...
def align_crop_from_preprocessed(
    ...
    if save_template_path is not None:
        # For bead / innerwall / tread cropping-only mode,
        # save final crop instead of full aligned image.
...
def build_calibration_pipeline(
    ...
    for idx, item in enumerate(processing_items):
        ...
        # IMPORTANT:
        # pre_bgr is defined here before align_crop_from_preprocessed().
        _raw_bgr, pre_bgr = read_and_polarize(raw_path)

        # IMPORTANT:
        # Innerwall uses reference_band_info, not reference_r/crop_anchor.
        ...
        # For def* calibration images, remove only known defective RC patches.
        # Remaining good patches can be used for bank/threshold.
        if is_defect_calib:
...
def load_runtime(
    ...
    # reference_band_info is already loaded from reference_band_info.pt
    # Only rebuild as fallback if it is missing.
    if reference_band_info is None:
